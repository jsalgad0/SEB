...
<body style="overflow: hidden;">
	<form:form name='aseguradorForm' action="eliminarAsegurador" method='POST' commandName="aseguradorForm" id="aseguradorForm">
		<form:hidden path="error"/>
		<form:hidden path="exito"/>
		<form:hidden path="editar"/>
		<form:hidden path="busqueda"/>
		<form:hidden path="idAsegurador"/>
...
<script type="text/javascript">
	function eliminar(){
		$("#aseguradorForm").attr("action", "eliminarAsegurador");
		$("#aseguradorForm").submit();
	}
	
	function buscar(){
		$("#aseguradorForm").attr("action", "buscarAseguradorEliminar");
		$("#aseguradorForm").submit();
	}
	...
	$( document ).ready(function() {
		...
		$('#asegurador').prop('readonly', false);
	...
	function inicializaAutocomplete(){
		...
		source: "buscarAseguradorDescripcion",
 	   	minLength: 3,
        select: function( event, ui ){
        	if (ui.item.aseguradorId!=-1) {
       	    	$("#asegurador").val(ui.item.nombreRazonSocial);
       	    	$("#idAsegurador").val(ui.item.aseguradorId);
       	    	$("#busqueda").val(1);
       	    	buscar();
			}else{
       	    	$("#asegurador").val("");
